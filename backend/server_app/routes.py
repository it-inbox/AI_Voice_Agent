...
from .config import CALL_HANDLER_URL, CallOutcome, PLIVO_ANSWER_URL, PLIVO_AUTH_TOKEN, PORT, SEND_QUEUE_MAXSIZE, check_user_auth, log
...
async def resolve_call_uuid(request: web.Request) -> web.Response:
    ...
    if not await check_user_auth(request):

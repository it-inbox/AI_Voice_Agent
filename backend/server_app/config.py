...
from groq import AsyncGroq   # MIGRATION: LLM leg runs on Groq (already used elsewhere in this codebase — see call_handler_app/config.py)
...
GROQ_API_KEY      = os.getenv("GROQ_API_KEY") or ""     # MIGRATION: NEW — we now own the LLM call directly, on Groq
...
if not GROQ_API_KEY:
    raise ValueError("GROQ_API_KEY missing")   # MIGRATION: was implicit via Deepgram's think passthrough
...
groq_client     = AsyncGroq(api_key=GROQ_API_KEY)              # MIGRATION: NEW — LLM leg (understands the STT
                                                                 # transcript and replies) now runs on Groq directly,
                                                                 # instead of via Deepgram's think passthrough.

# MIGRATION: NEW — raw STT turn-detection tuning (Voice Agent API did this internally)
UTTERANCE_END_MS   = 1000   # silence gap before a turn is considered done
...
AMD_MAX_WAIT_S            = 8.0
WARNING_DURATION_S        = 3 * 60   # CHANGED: was 4*60
MAX_CALL_DURATION_S       = 4 * 60   # CHANGED: was 6*60
